...
[motors_1s motors_5s motors_10s] = GetRealMotors_m1(data, +1, 8);
...
figure(1);clf;plot(1:length(Po),Po); axis([0 length(Sa) 0 0.5])
...
for (n=2:length(AllObservations));
    ...
    figure(1); plot(1:length(Po),Po); axis([0 length(Sa) 0 0.5]); xlabel('Motors'); ylabel('P(h|m)')  %plot it
    figure(2);
    ...
    plot(n,sest(1),'k.'); ylabel('Infered distance (normalised) with highest posterior probability'); axis([0 N 0 1 ])
    %pause
end;  
hold off;