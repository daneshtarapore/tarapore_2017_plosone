...
data            = load('/home/danesh/argos3-foraging/docs/bayesian_inference/sensors/nbrsdataforBI_Aggregation_CIRCLE.out');
...
hold on
for i=1:N
   plot(0.5, AllObservations(1,i),'k.','markersize',10);
   % pause
end;
xlabel('- '); ylabel('Sensors')
...
figure(3);clf;plot(1:length(Po),Po); axis([0 length(Sa) 0 0.5])
...
K=[1]; % variance of 1D distribution
...
for (n=2:length(AllObservations));
    ...
    for (i=1:length(Pr))
           me=[Sa(i)];
           m(i) = 1/(sqrt((2*pi)* K)) * exp(-((AllObservations(n)-me)^2)/(2 * K) ); %Compute likelihood      
    ...
    figure(3); plot(1:length(Po),Po); axis([0 length(Sa) 0 0.5]); xlabel('Sensors'); ylabel('P(h|s)')  %plot it
    figure(4);
    
    
    [a]=find(Po==max(max(Po)));  % Get the peak value - the MAP - the maximum a posteriori value.
    %USE THE EXPECTED VALUE INSTEAD OF THE MAP
    
    
    sest=[Sa(a)];  %A store the coordinates of this location in the bushes
    plot(n,sest(1),'k.'); ylabel('Infered number of neighbours (normalised) with highest posterior probability'); axis([0 N 0 1 ])
    %pause
end;  
hold off;