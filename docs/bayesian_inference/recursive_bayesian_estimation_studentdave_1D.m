...
% example modified by Danesh for a 1D map of the quail.


figure(1);clf;
figure(2);clf;
N = 500;
s=[4];  % where the quail is hiding
...
n=2*randn(1,N); % Create vector of iterative squawks with a standard deviation of 2.
x=zeros(1,N); % x will be the initialized variable for where the ninja thinks he hears the quail.
...
h=plot(s(1),5,'r.');  % Plot where the quail actually is
...
figure(1);clf;plot(1:length(Po),Po), axis([0 40 0 0.05])
...
for (n=2:length(x));
    ...
    figure(1);plot(1:length(Po),Po), axis([0 40 0 0.05]) %plot it
    figure(2);
    [a]=find(Po==(max(Po)));  % Get the peak value; it's most likely location of the Quail.
    sest=[Sa(a)];  %A store the coordinates of this location in the bushes
    plot(n,sest(1),'k.');axis([0 N 2 4 ])    
    %pause
end;  
hold off;